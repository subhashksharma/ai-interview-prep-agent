...
export default function QuickGeneralQuestions({ onComplete, onExit }: QuickGeneralQuestionsProps) {
  ...
  const requiredQuestions = quickQuestions.filter((q) => q.required !== false);
  const requiredCompleted = requiredQuestions.filter((q) => isQuestionFilled(q, answers)).length;
  const progress = requiredQuestions.length
    ? Math.round((requiredCompleted / requiredQuestions.length) * 100)
    : 100;
  const allRequiredAnswered = requiredQuestions.every((q) => isQuestionFilled(q, answers));
  ...
  return (
    <motion.section
      ...
      <div className='fixed inset-0 -z-10 overflow-hidden pointer-events-none'>
        <div className='absolute top-0 left-1/4 w-[600px] h-[600px] bg-[#2dcbc5]/8 rounded-full blur-3xl' />
        <div className='absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-blue-500/5 rounded-full blur-3xl' />
        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-[#2dcbc5]/3 to-violet-500/3 rounded-full blur-3xl animate-pulse' />
      </div>
          ...
          {quickQuestions.map((question, index) => {
            ...
            return (
              <motion.div
                ...
                {/* Card */}
                <div
                  className={`relative overflow-hidden rounded-2xl sm:rounded-3xl bg-white/80 backdrop-blur-sm border transition-all duration-300 ${
                    isFilled
                      ? 'border-[#2dcbc5]/30 shadow-lg shadow-[#2dcbc5]/10'
                      : isError
                      ? 'border-rose-300 shadow-lg shadow-rose-100'
                      : 'border-slate-200/60 shadow-lg shadow-slate-200/30 hover:border-slate-300/80 hover:shadow-xl'
                  }`}>
                            ...
                            return (
                              <motion.button
                                ...
                                className={`relative flex items-center gap-3 p-4 rounded-xl text-left transition-all duration-200 ${
                                  isSelected
                                    ? 'bg-gradient-to-br from-[#2dcbc5]/10 to-[#2ab7ca]/5 border-2 border-[#2dcbc5]/50 shadow-md shadow-[#2dcbc5]/10'
                                    : 'bg-slate-50/80 border-2 border-transparent hover:bg-slate-100/80 hover:border-slate-200'
                                }`}>
                                {option.icon && (
