    ...
    questions.push({
      ...
      options:
        questionType === 'single-choice' || questionType === 'multiple-choice'
          ? [
              'Option A: [LLM generated option]',
              'Option B: [LLM generated option]',
              'Option C: [LLM generated option]',
              'Option D: [LLM generated option]',
            ]
          : undefined,
      blanksCount: questionType === 'fill-in-blanks' ? 3 : undefined,
      caseStudyText:
        questionType === 'case-study'
          ? `This is a case study scenario about ${topic.name}. [LLM would generate detailed context here]`
          : undefined,
    });
  }
...
export async function generateCustomQuestions(input: CustomAssessmentInput): Promise<Question[]> {
  ...
  for (let i = 0; i < input.questionCount; i++) {
    questions.push({
      ...
      type: i % 3 === 0 ? 'writing' : i % 3 === 1 ? 'case-study' : 'coding',
      difficulty: input.difficulty,
...
export async function evaluateAnswer(
  ...
  return {
    ...
    encouragement: isPositive
      ? "Great work! You're showing solid understanding. Keep it up! 🌟"
      : "Good effort! Every attempt is a learning opportunity. You're making progress! 💪",
    nextSteps: [
