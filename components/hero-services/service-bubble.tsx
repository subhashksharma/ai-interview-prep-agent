...
export default function ServiceBubble({
  ...
  // Mobile: fixed width & height so all bubbles look identical.
  // Desktop: fixed height (h-9) but allow variable width with a sensible min-width.
  const pillSize = isMobileCompact ? 'w-36 h-9 text-xs px-3 py-1' : 'min-w-[140px] h-9 text-sm px-3 py-2';
    ...
    <motion.div
      ...
      <div
        ...
        style={{ animationDelay: `${delay + 0.55}s` }}>
            {href ? (
              <Link href={href} aria-label={label} style={gradientStyle} className={pillClasses}>
                <span className={dotClasses} style={{ background: dotColor }} />
                <span className={isMobileCompact ? 'truncate text-center w-full' : 'truncate'}>{label}</span>
              </Link>
            ) : (
              <div style={gradientStyle} className={pillClasses}>
                <span className={dotClasses} style={{ background: dotColor }} />
                <span className={isMobileCompact ? 'truncate text-center w-full' : 'truncate'}>{label}</span>
              </div>
            )}
